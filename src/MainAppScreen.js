...
const MainAppScreen = () => {
  ...
  const handleLogin = (name, email) => {
    // Aseguramos que el usuario se inicialice completamente si faltan datos
    setCurrentUser(prev => ({ 
        ...initialUser, // Usamos la base para evitar nulos
        ...prev, 
        name: name || prev.name, 
  ...
  const handleLogout = () => {
    setIsLoggedIn(false);
    setActiveTab('home'); 
    // Mantenemos currentUser, pero el estado de login lo oculta
  };
  ...
  const renderContent = () => {
    // Control de seguridad por si currentUser está indefinido (evita la pantalla en blanco)
    if (!currentUser) {
        return <div className="p-10 text-center text-gray-500">Error: Datos de usuario no disponibles.</div>;
    }
    ...
    switch (activeTab) {
      ...
      case 'profile':
        return (
            // Pasamos la función de actualización (handleUpdateUser) a ProfileScreen
            <ProfileScreen 
                user={currentUser} 
