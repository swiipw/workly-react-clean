...
// Datos simulados (añadimos age y preferences al usuario inicial)
const initialUser = { 
    name: "Usuario Ejemplo", 
    email: "ejemplo@workly.com",
    age: 25,
    preferences: 'Tecnología, Remoto'
};


const MainAppScreen = () => {
  const [isLoggedIn, setIsLoggedIn] = useState(false); 
  // CLAVE: Establecemos el estado inicial con más campos
  const [currentUser, setCurrentUser] = useState(initialUser); 
  
  const [activeTab, setActiveTab] = useState('home'); 
  
  const handleLogin = (name, email) => {
    // Cuando el usuario se loguea, usamos sus datos o los datos por defecto
    setCurrentUser(prev => ({ 
        ...prev, 
        name: name || prev.name, 
  ...
  // CLAVE: Nueva función para actualizar el perfil
